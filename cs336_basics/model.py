...
class TransformerLm(nn.Module):
    def __init__(self,vocab_size: int,context_length: int,d_model: int,num_layers: int,num_heads: int,d_ff: int,rope_theta: float,):
        super().__init__()
        self.token_embeddings=Embedding(vocab_size,d_model) #Shape is (vocab_size, d_model).
    ...
    def forward(self,x:torch.Tensor)->torch.Tensor:
        #token_ids → token_embeddings → 依序通過每一層 layers → ln_final → lm_head → logits
