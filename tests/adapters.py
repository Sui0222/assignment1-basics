...
def run_multihead_self_attention_with_rope(
    ...
    layer.load_state_dict({"q_proj.weight":q_proj_weight,"k_proj.weight":k_proj_weight,"v_proj.weight":v_proj_weight,"o_proj.weight":o_proj_weight})
    output=layer(in_features,token_positions)
    return output
    raise NotImplementedError
...
def run_transformer_block(
    ...
    block_weights["attn.o_proj.weight"] = block_weights.pop(
        "attn.output_proj.weight"
    )
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    from cs336_basics.bpe import train_bpe_optimized
    return train_bpe_optimized(str(input_path), vocab_size, special_tokens)
    raise NotImplementedError
